...
const addressLookupLimit = rateLimit({ namespace: 'customer-address-lookup', limit: 30, windowSeconds: 10 * 60, key: (request) => String((request as { auth?: { userId?: number } }).auth?.userId || request.ip) })
customerRoutes.get('/profile', customerController.getProfile)
customerRoutes.get('/account-data', customerController.exportData)
customerRoutes.delete('/account', customerController.deleteAccount)
customerRoutes.get('/address-suggestions', addressLookupLimit, customerController.searchAddressSuggestions)
customerRoutes.get('/current-address', addressLookupLimit, customerController.resolveCurrentAddress)
...
customerRoutes.post('/support-tickets', orderWriteLimit, customerController.createSupportTicket)
...
customerRoutes.get('/privacy/export', customerController.exportPersonalData)
customerRoutes.delete('/privacy/account', orderWriteLimit, customerController.deletePersonalAccount)
