...
const handoffOptions = new Set(['leave_at_door', 'meet_at_door', 'meet_outside'])
...
function normalizeAddress(payload: Record<string, unknown>): CustomerAddressPayload {
...
function normalizePreferences(payload: Record<string, unknown>): CustomerPreferencesPayload {
  const deliveryHandoff = trim(String(payload.deliveryHandoff || 'leave_at_door'))
  const preferredDeliveryWindow = trim(String(payload.preferredDeliveryWindow || 'asap'))
  ...
  return {
    ...
    orderStatusNotifications: booleanValue(payload.orderStatusNotifications, true),
    courierMessageNotifications: booleanValue(payload.courierMessageNotifications, true),
    promotionNotifications: booleanValue(payload.promotionNotifications, false),
    receiptEmailNotifications: booleanValue(payload.receiptEmailNotifications, true),
    twoStepVerification: booleanValue(payload.twoStepVerification, false),
    personalizedRecommendations: booleanValue(payload.personalizedRecommendations, true),
...
function normalizePaymentMethod(
  ...
  if (hasCardNumber && submittedLast4 && submittedLast4 !== last4) {
    throw new HttpError(400, 'Card number does not match the submitted last four digits.')
  }

  if (!label || !brand || !/^\d{4}$/.test(last4)) {
    throw new HttpError(400, 'Payment label, brand, and card number are required.')
  }
  ...
  return {
    ...
    encryptedCardNumber: hasCardNumber ? encryptPaymentCardNumber(cardNumber) : undefined,
    expMonth,
    expYear,
  }
}
...
export async function updateProfile(userId: number, payload: Record<string, unknown>) {
...
export async function updatePreferences(userId: number, payload: Record<string, unknown>) {
  const preferences = await customerRepository.updatePreferences(userId, normalizePreferences(payload))
  return { preferences }
}

export async function createAddress(userId: number, payload: Record<string, unknown>) {
  const address = await customerRepository.createAddress(userId, normalizeAddress(payload))
...
export async function updateAddress(
  ...
  const address = await customerRepository.updateAddress(userId, addressId, normalizeAddress(payload))
...
export async function createPaymentMethod(userId: number, payload: Record<string, unknown>) {
...
export async function setDefaultPaymentMethod(userId: number, paymentMethodId: number) {
  const paymentMethod = await customerRepository.setDefaultPaymentMethod(userId, paymentMethodId)
...
export async function deletePaymentMethod(userId: number, paymentMethodId: number) {
  const paymentMethod = await customerRepository.deletePaymentMethod(userId, paymentMethodId)
