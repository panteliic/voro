...
export function LiveOperationsMap({ orders }: { orders: OperationsOrder[] }) {
  ...
  return <div className="h-[22rem] overflow-hidden border-b border-line sm:h-[28rem]"><MapContainer center={points[0] || [44.816, 20.46]} className="size-full" scrollWheelZoom zoom={12}><TileLayer attribution='&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors &copy; <a href="https://carto.com/attributions">CARTO</a>' url="https://{s}.basemaps.cartocdn.com/rastertiles/voyager/{z}/{x}/{y}{r}.png" />{orders.map((order) => <div key={order.id}>{order.restaurantLatitude !== null && order.restaurantLongitude !== null ? <Marker icon={restaurantIcon} position={[order.restaurantLatitude, order.restaurantLongitude]}><Popup><strong>{order.restaurantName}</strong><br />{t('operations.pickup', { id: order.id })}</Popup></Marker> : null}{order.customerLatitude !== null && order.customerLongitude !== null ? <Marker icon={customerIcon} position={[order.customerLatitude, order.customerLongitude]}><Popup><strong>{order.customerName}</strong><br />{t('operations.delivery', { id: order.id })}</Popup></Marker> : null}{order.courierLatitude !== null && order.courierLongitude !== null ? <Marker icon={courierIcon} position={[order.courierLatitude, order.courierLongitude]}><Popup><strong>{order.courierName || t('operations.courier')}</strong><br />{t('operations.order', { id: order.id })} · {order.deliveryStatus || t('operations.assign')}</Popup></Marker> : null}</div>)}<FitBounds points={points} /></MapContainer></div>
}
